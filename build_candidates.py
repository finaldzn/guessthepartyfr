...
def main():
    ...
    # ---- worker/seed.sql --------------------------------------------------
    # Idempotent seed for the Cloudflare D1 `candidates` table.
    # The Worker reads from this table to issue rounds and validate answers,
    # so the canonical (name, party, role) lives server-side and never
    # appears on the client until /answer responds.
    ...
    with open(SEED, "w", encoding="utf-8") as f:
        ...
        f.write("DELETE FROM candidates;\n")
        # Batch in chunks of 200 rows per multi-VALUES INSERT to stay
        # comfortably under any statement-size limit.
        CHUNK = 200
        for i in range(0, len(out), CHUNK):
            chunk = out[i:i + CHUNK]
            f.write("INSERT INTO candidates (id, name, party, role, image_url, source) VALUES\n")
            lines = []
            for c in chunk:
                lines.append(
                    f"  ({c['id']}, "
                    ...
                    f"{sql_quote(c['party'])}, "
                    f"{sql_quote(c['role'])}, "
                    f"{sql_quote(c['image_url'])}, "
                    f"{sql_quote(c['source'])})"
                )
            f.write(",\n".join(lines))
            f.write(";\n")
        f.write("COMMIT;\n")
    print(f"✓ wrote {SEED} with {len(out)} INSERT rows")
